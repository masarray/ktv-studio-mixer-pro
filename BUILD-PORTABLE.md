...
4. The result is `release\sonkupik_karaoke.exe`.
...
prebuilt Windows N-API binaries supplied by `node-hid` and `serialport`, then
tests both modules inside the actual Electron runtime before packaging.
